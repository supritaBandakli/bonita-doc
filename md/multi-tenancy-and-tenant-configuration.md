...
Note that the security against CSRF attacks is enabled by default for all fresh installations, so that the subsequence REST API calls using DELETE, POST, or PUT HTTP methods must also define the `X-Bonita-API-Token` header, with the value transmitted via the associated cookie.

The above call saved the cookies on the disk, in the saved_cookies.txt:
    ...
    jjargot@Tetrapharmakon:~/Documents/project/training_750
